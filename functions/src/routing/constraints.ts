...
export type ConstraintScopeKind = 'this_cycle' | 'n_cycles' | 'until_cleared' | 'permanent'
...
const SCOPES: ConstraintScopeKind[] = ['this_cycle', 'n_cycles', 'until_cleared', 'permanent']

export function overlayFromConstraints(constraints: StoredConstraint[]): RoutingOverlay {
...
export function sanitizeIntent(raw: unknown): RoutingIntent | null {
  ...
  const summary =
    typeof data.summary === 'string' && data.summary.trim()
      ? data.summary.trim()
      : defaultSummary(action as ConstraintAction, resourceId, value, scope, nCycles)
  return {
