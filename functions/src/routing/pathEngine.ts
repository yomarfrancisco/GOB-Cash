...
export function explainPathPick(rank: TightnessRank, residual?: PathResidual | null): string {
  ...
  if (residual && rank.residualFit) {
    return `${pair} — leftover ${formatZar(residual.amountZar)} (${residual.economicPaymentId}) still has to move, so this onion is the next live legal pair rather than the last failed rail.`
  }
...
function pairKey(cardId: number, machineId: number): string {
  return `${cardId}:${machineId}`
}

function splitFlowAmounts(total: number, parts: number, minAmount: number, maxAmount: number): number[] {
  if (parts <= 0) return []
  const totalCents = Math.round(total * 100)
  const minCents = Math.round(minAmount * 100)
  const maxCents = Math.round(maxAmount * 100)
  const base = Math.floor(totalCents / parts)
  const remainder = totalCents - base * parts
  const amounts = Array.from({ length: parts }, (_, i) => base + (i < remainder ? 1 : 0))
  for (let i = 0; i < amounts.length; i++) {
    if (amounts[i] < minCents || amounts[i] > maxCents) return []
  }
  return amounts.map((cents) => cents / 100)
}

/** Linearized Pickhardt split: residual U across least-tight legal onions. Hold if the min-cut is below U. */
export function planFlow(params: {
  state: RoutingState
  overlay: RoutingOverlay
  cycleNumber: number
  amountZar: number
  ...
  | { onions: FlowOnion[]; ranks: TightnessRank[]; deployedAmount: number; holdReason?: undefined }
  | { onions: []; ranks: TightnessRank[]; deployedAmount: 0; holdReason: string } {
  ...
  const lastPair = params.residual ? pairKey(params.residual.cardId || 0, params.residual.machineId || 0) : ''
  const pairs = legalPairs(params.state, params.overlay, notes).filter((row) => !blocked.has(row.cardId))
  const ranks = pairs
    .map((row) =>
      scoreLegalPair({
        state: params.state,
  ...
  if (!ranks.length) {
    return { onions: [], ranks, deployedAmount: 0, holdReason: 'Hold: every legal pair is frozen or banned.' }
  }
  const min = params.state.config.minCardAmount
  const max = params.state.config.maxCardAmount
  const amount = Math.round(params.amountZar * 100) / 100
  if (amount + 0.005 < min) {
    return {
      onions: [],
      ranks,
      deployedAmount: 0,
      holdReason: `Hold: leftover ${formatZar(amount)} does not fit ${formatZar(min)}–${formatZar(max)}.`,
    }
  }
  if (ranks[0].tightness >= 8) {
    return {
      onions: [],
      ranks,
      deployedAmount: 0,
      holdReason: 'Hold: every legal pair is too tight from confirmed declines or unpaid leftovers to issue this evening.',
    }
  }
  const onionCount = Math.max(1, Math.ceil(amount / max))
  const capacity = onionCount * max
  if (amount - 0.005 > capacity) {
    return {
      onions: [],
      ranks,
      deployedAmount: 0,
      holdReason: `Hold: residual ${formatZar(amount)} is above the live min-cut ${formatZar(capacity)}.`,
    }
  }
  const ordered = [
    ...ranks.filter((row) => pairKey(row.cardId, row.machineId) !== lastPair),
    ...ranks.filter((row) => pairKey(row.cardId, row.machineId) === lastPair),
  ]
  const chosen: TightnessRank[] = []
  const usedCards = new Set<number>()
  const usedMachines = new Set<number>()
  for (const row of ordered) {
    if (chosen.length >= onionCount) break
    if (usedCards.has(row.cardId) || usedMachines.has(row.machineId)) continue
    chosen.push(row)
    usedCards.add(row.cardId)
    usedMachines.add(row.machineId)
  }
  if (chosen.length < onionCount) {
    return {
      onions: [],
      ranks,
      deployedAmount: 0,
      holdReason: `Hold: residual ${formatZar(amount)} needs ${onionCount} onions and the live min-cut is only ${chosen.length}.`,
    }
  }
  const amounts = splitFlowAmounts(amount, chosen.length, min, max)
  if (!amounts.length) {
    return {
      onions: [],
      ranks,
  ...
  return {
    ...
    onions: chosen.map((row, index) => ({
      cardId: row.cardId,
      machineId: row.machineId,
      amount: amounts[index],
      posReason: explainPathPick(row, params.residual),
    })),
  }
}
...
export function parsePathBook(raw: unknown): PathBook {
  ...
  const row = raw as {
    ...
    exhaustionNotes?: unknown
    lastShockLine?: unknown
  }
  return {
    ...
    lastShockLine: typeof row.lastShockLine === 'string' && row.lastShockLine ? row.lastShockLine : undefined,
  }
}
  ...
  const summary =
    kind === 'freeze'
      ...
      : kind === 'rail_up'
        ...
        : kind === 'decline'
          ? `${rail || 'Open ticket'}${amountBit} declined — leftover will reroute on the next live flow.`
          : kind === 'unpaid'
